...
components = construct_particles('GADGET halos', 'dark matter', N, mass)
posx = zeros(N)
posy = zeros(N)
posz = zeros(N)
momx = zeros(N)
...
for i in range(int(round(N**(1/3)))):
    for j in range(int(round(N**(1/3)))):
        for k in range(int(round(N**(1/3)))):
            ...
            count += 1
components.populate(posx, 'posx')
components.populate(posy, 'posy')
components.populate(posz, 'posz')
components.populate(momx, 'momx')
components.populate(momy, 'momy')
