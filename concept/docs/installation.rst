...
.. code-block:: bash
   ...
   concept_version=master
   mpi_dir=/path/to/mpi bash <(wget -O- https://raw.githubusercontent.com/jmd-dk/concept/${concept_version}/installer)
...
.. tip::
   ...
   simply ``mpi_dir=$(which mpicc)`` (though ``which mpicc`` does not
   evaluate exactly to ``/path/to/mpi``, it is close enough so that the
   ``installer`` understands it).
